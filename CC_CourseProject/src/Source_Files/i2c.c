...
static I2C_STATE_MACHINE i2c_sm_v;
...
void i2c_start(I2C_TypeDef*i2c, uint32_t address_slave, uint32_t command, uint32_t *data_, uint32_t callback, uint32_t numbytes, bool busy, bool read){
	...
	if(i2c == I2C1){
		...
		i2c_sm_v.I2C_periph = i2c;
		i2c_sm_v.slave_address = address_slave;
		i2c_sm_v.command_ = command;
		i2c_sm_v.data = data_;
		i2c_sm_v.callback = callback;
		i2c_sm_v.numbytes = numbytes;
		i2c_sm_v.busy = busy;
		i2c_sm_v.read = read;		//Read is true, Write is false
		i2c_sm_v.state = start_comm;
	}
...
void I2C1_IRQHandler(){
	uint32_t int_flag1 = I2C1->IF & I2C1->IEN;
	I2C1->IFC = int_flag1;

	if(int_flag1 & I2C_IF_ACK){
		i2c_ack(&i2c_sm);
	}
	if(int_flag1 & I2C_IF_NACK){
		i2c_nack(&i2c_sm);
	}
	if(int_flag1 & I2C_IF_RXDATAV){
		i2c_rxdatav(&i2c_sm);
	}
	if(int_flag1 & I2C_IF_MSTOP){
		i2c_mstop(&i2c_sm);
	}
		...
		if(i2c_state_m->read){
			switch(i2c_state_m->state){
					...
					case command:
						...
						break;
					case slave_rw:
							if(i2c_state_m->I2C_periph == I2C1){
			...
			case start_comm:
					...
					case MS_byte:
						if(i2c_state_m->numbytes == 2){
							...
							if(i2c_state_m->I2C_periph == I2C1){							//Si7021
								i2c_state_m->state = LS_byte;
					...
					case LS_byte:
						...
						if(i2c_state_m->I2C_periph == I2C1 || i2c_state_m->numbytes == 1){					//Si7021
								i2c_state_m->state = end_comm;
								*i2c_state_m->data |= (i2c_state_m->I2C_periph->RXDATA);
								i2c_state_m->I2C_periph->CMD = I2C_CMD_NACK;
								i2c_state_m->I2C_periph->CMD = I2C_CMD_STOP;
